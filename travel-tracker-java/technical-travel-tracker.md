...
> **Plataforma:** `Android`
> **Linguagem / Framework:** Java / Android Views com XML
...
<!-- ex.: MVVM, MVI, Clean Architecture, TCA, VIPER, Clean Architecture -->
...
Além disso, o MVVM possui boa aderência às práticas modernas do Android, sendo compatível com componentes como ViewModel, LiveData, Retrofit, ExecutorService, Room e bibliotecas de persistência local. Isso torna sua implementação mais simples, padronizada e alinhada com aplicações Android utilizadas no mercado.
...
│                                                            │
│  Activities / Fragments / layouts XML / Views              │
│  ViewModels                                                │
...
### 1.4 Stack Android revisada — Java + XML

A implementação Android deve utilizar a seguinte stack como padrão obrigatório:

| Categoria | Decisão técnica |
|-----------|-----------------|
| Linguagem | Java |
| Interface | Layouts XML com Material Components |
| Arquitetura | MVVM |
| Camada visual | Activity única ou host principal com Fragments por feature |
| Navegação | AndroidX Navigation Component com `nav_graph.xml` |
| Binding de tela | ViewBinding obrigatório; DataBinding opcional apenas quando justificado |
| Estado de tela | ViewModel expondo `LiveData<UiState>` |
| Eventos únicos | `SingleLiveEvent`, `Event<T>` wrapper ou canal equivalente baseado em LiveData |
| Assíncrono | Retrofit `Call<T>`/callbacks, `ExecutorService`, `Handler` e APIs Java compatíveis |
| Banco local | Room com DAOs retornando `LiveData` ou operações síncronas executadas em background |
| Sessão segura | EncryptedSharedPreferences |
| Preferências simples | SharedPreferences |
| Background location | Foreground Service em Java com notificação persistente |

Não devem ser usados Jetpack Compose, arquivos `.kt`, Composables, `StateFlow`, `SharedFlow`, `Flow` ou Coroutines na implementação desta versão. Quando uma dependência oferecer exemplos em Kotlin, a implementação deve ser adaptada para Java.
...
|---------|------------------------------|---------------------------|-----------------|
| FEAT-01 | Autenticação e sessão do usuário | Implementar login por e-mail e senha, validação de sessão existente, armazenamento seguro de `accessToken` e `refreshToken`, renovação de sessão quando aplicável e redirecionamento automático para login ou tela principal. | [feature-login.md](./features/feature-login.md) |
| FEAT-02 | Cadastro de novo usuário | Implementar cadastro de usuário com validação de nome completo, e-mail único e senha forte. Após cadastro com sucesso, retornar para a tela de login. | [feature-novo-usuario.md](./features/feature-novo-usuario.md) |
| FEAT-03 | Recuperação e alteração de senha | Implementar fluxo de recuperação de senha por e-mail, envio do e-mail para API, recebimento e validação do token, definição de nova senha e retorno para login após sucesso. | [feature-recuperar-senha.md](./features/feature-recuperar-senha.md) |
| FEAT-04 | Tela principal e navegação por abas | Implementar tela principal com abas para **Minhas Viagens** e **Viagens Acompanhadas**, respeitando sessão ativa, carregamento inicial e navegação para detalhes, mapa, perfil e ações principais. | [feature-tela-principal.md](./feature-tela-principal.md) |
...
| Viagem ativa atual | Room ou SharedPreferences | Pode ser salva em Room caso seja necessário manter vínculo com a entidade `Viagem`. Também pode ser salva em SharedPreferences se forem armazenados apenas identificadores simples, como `viagemId` e `status`. |
| Preferências não sensíveis do usuário | SharedPreferences | Indicado para preferências simples, como tema, última aba acessada, flags de onboarding e configurações não sensíveis. SharedPreferences deve ser usado para preferências simples e não sensíveis. Quando houver necessidade de notificar alterações, encapsular o acesso em um repositório que publique mudanças por LiveData. :contentReference[oaicite:1]{index=1} |
...
4. Dados sensíveis não devem ser salvos em Room, SharedPreferences comum ou arquivos não protegidos.
5. Viagens, viagens acompanhadas e coordenadas pendentes devem ser armazenadas em Room.
6. Preferências simples e não sensíveis devem ser armazenadas em SharedPreferences.
...
|----------------|--------|------|
| Estado efêmero de UI | Local da tela | Activity, Fragment ou Fragment / layout XML |
| Estado da tela | ViewModel da tela | ViewModel da feature |
| Estado da feature | ViewModel da feature | Módulo da feature |
...
| Preferências não sensíveis | Escopo do app ou usuário | SharedPreferences |
...
<!-- ex.: Combine / AsyncStream / LiveData / LiveData / RxSwift -->

| Plataforma | Primitivo | Uso |
|------------|-----------|-----|
| Android | Java Executors/Retrofit Callbacks | Execução assíncrona de chamadas de API, operações em banco local, validações e tarefas de sincronização. |
| Android | LiveData | Exposição de estado contínuo das telas a partir dos ViewModels. |
| Android | SingleLiveEvent ou Event wrapper com LiveData | Emissão de eventos únicos, como mensagens, navegação, abertura de modal e confirmação de ações. |
| Android | LiveData | Observação de dados locais vindos do Room, SharedPreferences, sessão e conectividade. |
...
| Android | SharedPreferences observável quando necessário | Observação de preferências não sensíveis do usuário. |
| Android | Room com LiveData | Observação reativa de dados locais, como viagens, viagens acompanhadas e coordenadas pendentes. |
| Android | LiveData | Usar como mecanismo principal de exposição de estado entre ViewModel e UI. |
...
3. Estados contínuos devem ser expostos preferencialmente com `LiveData`.
4. Eventos únicos devem ser expostos preferencialmente com `SingleLiveEvent ou Event wrapper com LiveData`.
5. Dados locais observáveis devem ser expostos como `LiveData`, preferencialmente retornado pelo Room ou adaptado no repositório.
6. Operações assíncronas devem ser executadas com Executors/Retrofit Callbacks.
...
| Dados sensíveis | Tokens não devem ser armazenados em banco local comum, SharedPreferences não criptografado ou arquivos simples. | Restringir tokens ao armazenamento seguro do Android. |
...
## 15. Diretrizes específicas para implementação Java + XML

### 15.1 Estrutura de pacotes recomendada

```text
br.com.chronus.traveltracker
├── app
│   ├── MainActivity.java
│   └── TravelTrackerApplication.java
├── core
│   ├── analytics
│   ├── common
│   │   ├── Result.java
...
### 15.2 Padrão de ViewModel

Cada ViewModel deve expor o estado da tela por `LiveData`:

```java
public class LoginViewModel extends ViewModel {
    private final MutableLiveData<LoginUiState> uiState = new MutableLiveData<>(LoginUiState.idle());
    private final MutableLiveData<Event<LoginNavigation>> navigation = new MutableLiveData<>();

    public LiveData<LoginUiState> getUiState() {
        return uiState;
    }

    public LiveData<Event<LoginNavigation>> getNavigation() {
        return navigation;
    }
}
```

### 15.3 Padrão de Fragment

Cada Fragment deve inflar seu layout XML via ViewBinding, observar o ViewModel e enviar ações do usuário:
...
As telas devem usar XML com Material Components: `TextInputLayout`, `TextInputEditText`, `MaterialButton`, `MaterialToolbar`, `RecyclerView`, `BottomNavigationView`, `TabLayout` quando aplicável e `ConstraintLayout` como layout base preferencial.

### 15.5 Navegação

A navegação deve ser declarada em `res/navigation/nav_graph.xml`. A `MainActivity` deve hospedar um `NavHostFragment`, e cada feature deve navegar usando `NavController`.

### 15.6 Assíncrono e chamadas de rede

As chamadas HTTP devem usar Retrofit em Java. Operações de banco e processamento devem ocorrer fora da Main Thread usando `ExecutorService`. A UI deve ser atualizada via `LiveData` no ViewModel.

### 15.7 Room em Java

DAOs devem ser compatíveis com Java. Para dados observáveis, preferir retorno `LiveData<List<Entidade>>`. Operações de insert/update/delete devem ser chamadas em background.

### 15.8 Critérios de aceite técnicos globais

- [ ] Nenhum arquivo `.kt` deve ser criado para a implementação principal.
- [ ] Nenhuma tela deve ser implementada com Jetpack Compose.
- [ ] Todas as telas devem possuir layout XML correspondente.
- [ ] Todas as telas devem usar MVVM com ViewModel Java.
- [ ] Estado de tela deve ser observado via LiveData.
- [ ] Eventos únicos de navegação/mensagem devem evitar reemissão após rotação.
- [ ] Repositórios devem isolar acesso à API, Room e sessão.
- [ ] Operações de I/O não devem executar na Main Thread.
