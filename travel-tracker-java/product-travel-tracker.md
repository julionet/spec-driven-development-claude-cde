...
## Anexo — Restrição de stack para implementação Android

A versão Android descrita por esta especificação deve ser implementada em **Java**, com telas em **XML** e arquitetura **MVVM**. A especificação de produto permanece funcionalmente igual, porém todos os documentos técnicos vinculados devem considerar esta restrição de stack.

Diretrizes obrigatórias:

- A interface deve ser criada com layouts XML em `res/layout`.
- A navegação deve usar Activities/Fragments e AndroidX Navigation Component.
- O estado das telas deve ser controlado por ViewModels Java usando LiveData.
- Chamadas de API devem ser feitas com Retrofit em Java.
- Persistência local deve usar Room com entidades, DAOs e repositórios compatíveis com Java.
- Captura contínua de localização deve ser feita por Foreground Service em Java.
- Não utilizar Jetpack Compose, Composables, Kotlin, Coroutines, Flow, StateFlow ou SharedFlow nesta versão.
