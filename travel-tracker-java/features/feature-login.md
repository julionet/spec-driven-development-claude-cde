...
| `TextInputLayout` + `TextInputEditText` | E-mail | Rótulo: "E-mail"; teclado de e-mail; validação de formato. |
| `TextInputLayout` + `TextInputEditText` senha | Senha com visibilidade alternável | Rótulo: "Senha"; opção para mostrar/ocultar senha. |
| `MaterialButton` | Padrão | Rótulo: "Entrar"; largura total; estado normal, carregando e desabilitado. |
...
|------------|------|-----------------|
| `LoginFragment` | Fragment / Tela XML | Renderiza a tela de login, campos de e-mail e senha, botão de entrada, links para cadastro e recuperação de senha. Observa o estado exposto pelo ViewModel e dispara eventos de interação do usuário. |
| `LoginViewModel` | ViewModel | Mantém o estado da tela de login, valida campos, controla carregamento, aciona o caso de uso de autenticação e emite eventos de navegação ou erro. |
...
1. `LoginFragment` exibe a tela de login.
2. O usuário informa e-mail e senha.
3. `LoginFragment` envia a ação para `LoginViewModel`.
