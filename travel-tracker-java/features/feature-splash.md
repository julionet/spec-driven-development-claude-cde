...
| `MaterialButton` | Padrão | Rótulo: "Tentar novamente", quando existir erro recuperável. |
...
|------------|------|-----------------|
| `SplashFragment` | Fragment / Tela XML | Renderiza a tela de Splash com logo centralizado, loading inferior e texto **"Carregando..."**. Observa o estado do `SplashViewModel` e executa a navegação conforme evento emitido. |
| `SplashViewModel` | ViewModel | Controla o estado inicial da Splash, executa verificações iniciais, chama casos de uso e emite eventos de navegação para Login ou Tela Principal. |
...
1. `SplashFragment` é exibida ao abrir o aplicativo.
...
| Viagem ativa atual | Sim | SharedPreferences, vinculada ao usuário logado | Permitir que o app saiba qual viagem está em andamento. |
| Coordenadas pendentes | Sim | Room / base local temporária | Manter coordenadas capturadas até o envio bem-sucedido para a API. |
| Preferências não sensíveis | Não | SharedPreferences | Guardar configurações simples, como última aba acessada ou preferências visuais. |
