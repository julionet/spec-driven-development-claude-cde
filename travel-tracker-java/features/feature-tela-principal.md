...
| Dados locais foram atualizados pelo Splash em segundo plano | A tela principal deve observar o banco local (`LiveData` do Room) e refletir as alterações automaticamente. |
...
| `ListaViagens` | Scrollável | Lista de cards `item_viagem.xml` ou `item_viagem_acompanhada.xml`. |
| `item_viagem.xml` | Card | Ícone do veículo, descrição, veículo, data de início, data de término, status. Ação: navegar para detalhes. |
| `item_viagem_acompanhada.xml` | Card | Nome do proprietário, ícone do veículo, descrição, veículo, data de início, data de término, status do convite. Ações: navegar para detalhes (se aceito) ou aceitar/rejeitar (se pendente). |
...
| `MaterialButton` | Flutuante / Fixo | Rótulo: "Nova viagem", visível apenas na aba "Minhas Viagens". |
...
|------------|------|-----------------|
| `HomeFragment` | Fragment / Tela XML | Renderiza a tela principal com barra superior, barra de navegação inferior com duas abas e conteúdo da aba ativa. Observa o estado do ViewModel e dispara eventos de interação. |
| `MinhasViagensFragment` | Fragment / layout XML | Renderiza o conteúdo da aba "Minhas Viagens": filter chips, lista de cards e botão "Nova viagem". |
| `ViagensAcompanhadasFragment` | Fragment / layout XML | Renderiza o conteúdo da aba "Viagens Acompanhadas": filter chips e lista de cards com ações de aceitar/rejeitar. |
| `item_viagem.xml` | Fragment / layout XML | Card individual de uma viagem própria com ícone, descrição, veículo, datas e status. |
| `item_viagem_acompanhada.xml` | Fragment / layout XML | Card individual de uma viagem acompanhada com nome do proprietário, descrição, veículo, datas e status do convite. Inclui botões de ação para convites pendentes. |
| `HomeViewModel` | ViewModel | Mantém o estado da tela principal, incluindo aba ativa, dados das listas, filtro selecionado em cada aba e badge de convites pendentes. |
...
| `ObserveMyTripsUseCase` | Caso de Uso | Observa o banco local (`LiveData` do Room) e retorna as viagens do usuário (`ViagemMinha`) filtrando conforme o chip selecionado (`ativas`, `finalizadas`, `canceladas`, `todas`). |
| `ObserveFollowedTripsUseCase` | Caso de Uso | Observa o banco local (`LiveData` do Room) e retorna as viagens acompanhadas (`ViagemAcompanhada`) filtrando conforme o chip selecionado (`aceitas`, `pendentes`, `todas`). |
...
| `TripLocalDataSource` | Fonte de Dados Local | Acessa as entidades `ViagemMinha` e `ViagemAcompanhada` no Room via DAO. Expõe `LiveData` para observação de dados. |
...
HomeFragment
  └─► HomeViewModel
        ...
        │     └─► HomeRepository
        │           └─► TripLocalDataSource (Room com LiveData → ViagemMinha)
        │                 └─► Banco local (pré-carregado pelo Splash)
        │
        ├─► ObserveFollowedTripsUseCase
        │     └─► HomeRepository
        │           └─► TripLocalDataSource (Room com LiveData → ViagemAcompanhada)
        │                 └─► Banco local (pré-carregado pelo Splash)
        │
...
1. `HomeFragment` é exibida após navegação do Splash ou Login.
2. `HomeViewModel` inicia `ObserveMyTripsUseCase` e `ObserveFollowedTripsUseCase`.
3. Cada caso de uso consulta o Room via LiveData (reativo) e aplica o filtro selecionado.
...
6. `HomeFragment` re-renderiza conforme o estado.
7. Quando o usuário altera o chip de filtro, o ViewModel reaplica o filtro sobre os dados observados via LiveData.
8. Quando o Splash atualiza os dados em segundo plano, o `LiveData` do Room emite novos valores e a tela é atualizada automaticamente.
...
- [ ] Dados reativos: alterações no Room (via Splash ou aceite/rejeição de convite) refletidas automaticamente na UI via LiveData.
