...
|------------|----------|--------------|
| `CampoTexto` | Padrão | Rótulo: "Nome", tipo: texto, capitalização: palavras, largura total |
| `CampoTexto` | Padrão | Rótulo: "E-mail", tipo: email, `keyboardType = Email`, sem capitalização, largura total |
| `CampoSenha` | Com alternância de visibilidade | Rótulo: "Senha", oculto por padrão, ícone de olho à direita, largura total |
| `MensagemErroInline` | Erro de campo | Exibida abaixo do campo correspondente; visível apenas quando há erro |
| `BotaoPrimario` | Padrão | Rótulo: "Salvar", largura total; estado de carregamento com `CircularProgressIndicator` |
...
|------------|------|-----------------|
| `CadastroUsuarioScreen` | Composable (View) | Renderiza TLA-01; observa `CadastroUsuarioUiState` do ViewModel |
...
// via SharedFlow no ViewModel; o caller apenas observa esse evento.
```
...
| Transição de estado: Carregando → Sucesso | `CadastroUsuarioViewModel` | Caso de uso retorna sucesso | Estado emitido = `Sucesso`; evento de navegação emitido via `SharedFlow` |
