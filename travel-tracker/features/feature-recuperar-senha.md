...
|------------|----------|--------------|
| `BotaoPrimario` | Padrão | Rótulo: "Enviar código" / "Validar código" / "Alterar senha", largura total |
| `CampoTexto` | E-mail | Placeholder: "seu@email.com", teclado tipo e-mail, validação de formato |
| `CampoTexto` | Senha | Ícone de exibir/ocultar, validação de requisitos |
| `CampoTexto` | Token | 1 dígito, entrada numérica, foco automático, máximo 1 caractere |
...
| `SolicitarEmailScreen` | View / Tela | Renderiza TLA-01 com campo de e-mail e botão "Enviar código". Observa o estado do ViewModel e dispara eventos de interação. |
| `ValidarTokenScreen` | View / Tela | Renderiza TLA-02 com 6 campos de dígito, link "Reenviar código" e botão "Validar código". Inclui lógica de foco automático entre campos. |
| `AlterarSenhaScreen` | View / Tela | Renderiza TLA-03 com campos "Nova senha" e "Confirmar senha", lista de requisitos e botão "Alterar senha". |
| `SolicitarEmailViewModel` | ViewModel / StateHolder | Mantém o estado da TLA-01, valida o e-mail localmente, aciona o caso de uso de solicitação de token e emite eventos de navegação ou erro. |
| `ValidarTokenViewModel` | ViewModel / StateHolder | Mantém o estado da TLA-02, controla os 6 campos de dígito e validação local, aciona o caso de uso de validação do token e emite eventos de navegação ou erro. |
| `AlterarSenhaViewModel` | ViewModel / StateHolder | Mantém o estado da TLA-03, valida os campos localmente (requisitos da senha, igualdade das confirmações), aciona o caso de uso de alteração e emite eventos de navegação ou erro. |
...
A feature de recuperação de senha não requer alterações na persistência local. O token recebido da API deve ser mantido apenas em memória durante o fluxo (no ViewModel) e descartado ao sair da feature. Nenhum dado de recuperação deve ser salvo em banco local, DataStore ou armazenamento seguro.
